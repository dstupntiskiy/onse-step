...
export class EventDialogComponent implements DynamicComponent {
  ...
  init() {
    ...
    this.substitution$ = this.initialEvent()?.id ? this.eventService.getCoachSubstitution(this.initialEvent()!.id) : of()
    ...
    if (this.form.valid) {
      ...
      if (this.isRecur?.value) {
        data.daysOfWeek = (this.weekdays?.value as Weekday[]).map((x: Weekday) => x.number),
          ...
          data.recurrenceId = this.initialEvent()?.recurrence?.id ?? '00000000-0000-0000-0000-000000000000'
      }
