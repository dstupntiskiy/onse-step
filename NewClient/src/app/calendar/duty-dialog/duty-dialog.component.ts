...
export class DutyDialogComponent implements DynamicComponent {
      ...
      if(this.data()?.id){
        ...
        this.eventService.GetEventDutyById(this.data()?.id!)
          ...
          .subscribe(result => {
            ...
            this.start.setValue(new Date(result.startDateTime))
            this.end.setValue(new Date(result.endDateTime))
  ...
  onSave(){
    if (!this.selectedDuty() || !this.start.value || !this.end.value || +this.end.value <= +this.start.value) {
      this.snackbarService.error('Выберите дежурного и время окончания позже начала');
      return;
    }
